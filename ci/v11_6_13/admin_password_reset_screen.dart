...
class _AdminPasswordResetScreenState extends State<AdminPasswordResetScreen> {
  ...
  Future<void> _deleteFor(AppUser profile) async {
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setDialogState) {
          final confirmationOk = confirmCtrl.text.trim().toUpperCase() == 'SUPPRIMER';

          Future<void> submit() async {
            if (deleting) return;
            if (!confirmationOk) {
              setDialogState(() => dialogError = 'Tapez SUPPRIMER pour confirmer la suppression définitive.');
              return;
            }
            ...
            try {
              ...
              ScaffoldMessenger.of(this.context).showSnackBar(
                SnackBar(
                  content: Text('Compte de ${profile.fullName} supprimé avec ses gardes et ses données.'),
                  backgroundColor: Colors.green.shade700,
                ),
              );
            } catch (e) {
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                      ...
                      style: TextStyle(color: Colors.red.shade900, fontWeight: FontWeight.w600, height: 1.4),
                    ),
                  ),
                  const SizedBox(height: 16),
                  TextField(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const GardeFlowTitle('Gestion des comptes')),
      body: Column(
        children: [
          Padding(
            ...
            child: AppCard(
              ...
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Icon(Icons.admin_panel_settings_rounded, color: AppColors.brand),
                                    ...
                                    trailing: Row(
                                      ...
                                      children: [
                                        IconButton(
                                          ...
                                          icon: const Icon(Icons.lock_reset_rounded, color: AppColors.brand),
                                        ),
                                        IconButton(
