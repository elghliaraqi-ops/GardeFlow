...
class AppState extends ChangeNotifier {
  ...
  String get backendModeLabel =>
      backendEnabled ? 'Supabase connecté' : 'Mode local';

  void _firePush(String kind, String resourceId) {
    if (!backendEnabled) return;
    unawaited(
      SupabaseBackendService.instance.triggerPush(kind, resourceId).catchError((
        Object e,
      ) {
        debugPrint('Envoi push impossible: $e');
      }),
    );
  }
  ...
  Future<bool> _syncOfficialRosterForProfile(AppUser profile) async {
    if (!backendEnabled || profile.accountStatus != AccountStatus.active)
      return false;
  ...
  Future<void> forceSyncMyOfficialRoster() async {
    ...
    if (slot == null) {
      throw StateError(
        'Aucun planning officiel configuré pour votre établissement.',
      );
    }
  ...
  Future<void> _syncMyOfficialRosterIfNeeded() async {
    ...
    try {
      ...
      debugPrint(
        'Synchronisation automatique du planning officiel impossible: $e',
      );
    }
  }
  ...
  Future<void> _reloadFromBackend() async {
    ...
    final passwordResetFuture = sessionUser.role == UserRole.admin
        ...
        : Future<List<PasswordResetRequest>>.value(
  ...
  void _scheduleRealtimeReload({bool syncOfficialRoster = false}) {
    _realtimeNeedsOfficialSync =
        _realtimeNeedsOfficialSync || syncOfficialRoster;
  ...
  List<PlanningEntry> entriesForDate(String d) =>
      _planning.where((e) => e.dateStr == d).toList();
  PlanningEntry? entryForUserOnDate(String d, String phone) => _planning
      .where((e) => e.dateStr == d && e.ownerPhone == phone)
      .firstOrNull;
  PlanningEntry? entryForUserIdOnDate(String d, String ownerId) => _planning
      .where((e) => e.dateStr == d && e.ownerId == ownerId)
      .firstOrNull;
  ...
  PlanningMonth? planningMonthForUser(String ownerId, int year, int month) =>
      _planningMonths
          .where(
            (p) => p.ownerId == ownerId && p.year == year && p.month == month,
          )
          .firstOrNull;
  ...
  PlanningMonthStatus planningMonthStatusForUser(
    String ownerId,
    int year,
    int month,
  ) =>
      planningMonthForUser(ownerId, year, month)?.status ??
      PlanningMonthStatus.draft;
  ...
  bool canEditMyPlanningMonth(DateTime month) {
    ...
    if (monthStart.isBefore(currentMonthStart) ||
        monthStart.isAfter(maxPlanningMonth)) {
      return false;
    }
  ...
  bool canAdminDeleteEntry(PlanningEntry entry) {
    ...
    return entry.isDisciplinary ||
        isPlanningEntryApproved(entry) ||
        isApprovedLeaveEntry(entry);
  }
  ...
  final Map<String, Map<String, dynamic>> _reminderPrefsByUser =
  ...
  int get delayMinutes =>
      _reminderDelays.isNotEmpty ? _reminderDelays.first : _delayMinutes;
  set delayMinutes(int v) {
    unawaited(setReminderDelays(<int>[v]));
  }

  bool get notificationsOn => _notificationsOn;
  set notificationsOn(bool v) {
    unawaited(setNotificationsOn(v));
  }
  ...
  void _applyReminderPrefsForCurrentUser() {
    ...
    final key = _reminderOwnerKey;
    if (key == null) return;
    final p = _reminderPrefsByUser[key];
    if (p == null) {
      // Migration douce de l'unique ancien réglage lors de la première ouverture.
      if (_reminderPrefsByUser.isEmpty) {
        _reminderDelays = <int>[legacyDelay];
        _notificationsOn = legacyOn;
      }
  ...
  Future<void> _saveReminderPrefs() async {
    _storeReminderPrefsForCurrentUser();
    await _persistNow();
    notifyListeners();
  }

  Future<void> setReminderDelays(List<int> values) async {
    final cleaned = values.where((e) => e > 0 && e <= 10080).toSet().toList()
      ..sort((a, b) => b.compareTo(a));
    _reminderDelays = cleaned.isEmpty ? <int>[60] : cleaned.take(3).toList();
    await _saveReminderPrefs();
  }

  Future<void> setReminderRepeatMinutes(int value) async {
    _reminderRepeatMinutes = value.clamp(5, 180);
    await _saveReminderPrefs();
  }

  Future<void> setReminderRepeatCount(int value) async {
    _reminderRepeatCount = value.clamp(0, 3);
    await _saveReminderPrefs();
  }

  Future<void> setReminderVibration(bool value) async {
  ...
  Future<void> testGuardReminder() async {
    await NotificationService.instance.showReminderTest(
      soundMode: _reminderSoundMode,
      vibration: _reminderVibration,
    );
  }
  ...
  int get currentFirstYearPromotion {
    ...
    return _currentFirstYearPromotion > 0
  ...
  Future<void> refreshPromotionConfig() async {
    if (!backendEnabled) return;
    try {
      final value = await SupabaseBackendService.instance
          .fetchCurrentFirstYearPromotion();
  ...
  List<DirectoryContact> get manualDirectoryContacts =>
      List.unmodifiable(_manualDirectoryContacts);

  final List<DirectorySection> _directory = [];
  List<DirectorySection> get directory => _directory;

  String _directoryPhoneKey(String value) =>
  ...
  void _rebuildDirectory() {
    ...
    for (final u in _users.where(
      ...
      final id = u.grade == MedicalGrade.senior
          ? kDirectoryCategorySeniors
          : kDirectoryCategoryJuniors;
      final section = _directory.where((s) => s.id == id).firstOrNull;
      final phoneKey = _directoryPhoneKey(u.phone);
      if (section != null &&
          !section.contacts.any(
            (c) => _directoryPhoneKey(c.phone) == phoneKey,
          )) {
  ...
  Future<String?> addDirectoryContact({
    ...
    if (!kManualDirectoryCategoryIds.contains(categoryId))
    ...
    if (duplicate)
      return 'Ce numéro existe déjà dans l’annuaire de cet établissement.';
    try {
      if (backendEnabled) {
      ...
      return 'Ajout du contact impossible : $e';
    }
  }
  ...
  Future<String?> updateDirectoryContact({
    ...
    try {
      ...
      return 'Modification du contact impossible : $e';
    }
  }
  ...
  Future<String?> deleteDirectoryContact(String id) async {
    ...
    if (me == null || me.role != UserRole.admin)
      return 'Action réservée à l’administrateur.';
    if (!_manualDirectoryContacts.any((c) => c.id == id))
      return 'Contact manuel introuvable.';
    try {
      if (backendEnabled) {
      ...
      return 'Suppression du contact impossible : $e';
    }
  }
  ...
  Future<void> addAstreintePhoto(XFile file) async {
    ...
    final mine = _astreinte.where((p) => p.ownerPhone == me.phone).toList()
      ..sort((a, b) => b.createdAt.compareTo(a.createdAt));
    if (mine.length > 8) {
      final keep = mine.take(8).map((p) => p.id).toSet();
      _astreinte.removeWhere(
        (p) => p.ownerPhone == me.phone && !keep.contains(p.id),
      );
    }
    await _persistNow();
    notifyListeners();
  ...
  void removeAstreintePhoto(String id) {
    final me = currentUser;
    if (me == null) return;
    _astreinte.removeWhere((p) => p.id == id && p.ownerPhone == me.phone);
    _persist();
    notifyListeners();
  }

  final List<ReminderNotification> _reminders = [];
  List<ReminderNotification> get reminders {
    _refreshReminderStatuses();
    final me = currentUser;
    if (me == null) return const [];
    return List.unmodifiable(_reminders.where((r) => r.ownerPhone == me.phone));
  }
  ...
  List<PasswordResetRequest> get passwordResetRequests =>
      List.unmodifiable(_passwordResetRequests);
  int _exchangeCounter = 0, _leaveCounter = 0, _reminderCounter = 0;

  LeaveRequest? pendingLeaveForUserOnDate(String d, String phone) =>
      _leaveRequests
          .where(
            (r) =>
                r.includesDate(d) &&
  ...
  Future<String?> login(String rawPhone, String password) async {
    ...
    if (_notificationsOn)
      unawaited(NotificationService.instance.requestPermission());
    unawaited(_activateNativeRemindersForCurrentUser());
    notifyListeners();
    return null;
  }
  ...
  Future<String?> register({
    ...
    if (backendEnabled) {
      try {
        final backend = SupabaseBackendService.instance;
        final pending = await backend.signUp(
          nom: nom,
          prenom: prenom,
          rawPhone: rawPhone,
          password: password,
    ...
    if (_users.any((u) => u.phone == phone))
      return 'Un compte existe déjà avec ce numéro. Connectez-vous.';
    ...
    final user = AppUser(
      ...
      prenom: prenom.trim(),
      phone: phone,
      passwordHash: PasswordService.hash(password, salt),
      passwordSalt: salt,
      service: service,
      grade: grade,
      hospital: hospital,
  ...
  Future<String?> placeShift(String dateStr, String shiftId) async {
    ...
    if (_isDateInPast(dateStr))
      return 'Une date passée ne peut plus être modifiée.';
    ...
    if (existing?.isDisciplinary == true)
      return 'Cette garde disciplinaire est verrouillée. Seul un administrateur peut la supprimer.';
    if (existing != null && isApprovedLeaveEntry(existing))
      return 'Ce congé a déjà été approuvé. Seul un administrateur peut le supprimer.';
    if (existing != null && _isEntryLocked(existing.id))
      return 'Cette garde est verrouillée par une demande active.';
    try {
      if (backendEnabled) {
        await SupabaseBackendService.instance.saveMyPlanningEntry(
          dateStr: dateStr,
          shiftId: shiftId,
        );
        await _reloadFromBackend();
  ...
  PlanningMonth _ensureLocalPlanningMonth(
    String ownerId,
    int year,
    int month,
    ...
    if (record == null) {
      ...
      if (status == PlanningMonthStatus.draft) {
  ...
  Future<String?> removeShift(String entryId) async {
    ...
    if (entry.isDisciplinary)
      return 'Cette garde disciplinaire ne peut pas être annulée. Seul un administrateur peut la supprimer.';
  ...
  Future<String?> submitMyPlanningMonth(DateTime month) async {
    ...
    try {
      if (backendEnabled) {
        final leaveIds = await SupabaseBackendService.instance
            .submitMyPlanningMonth(month.year, month.month);
        ...
        for (final id in leaveIds) {
          _firePush('leave_created', id);
        }
        await _reloadFromBackend();
      } else {
        final r = _ensureLocalPlanningMonth(
          me.id,
          month.year,
          month.month,
        ...
        while (i < conges.length) {
          ...
          _leaveRequests.add(
            LeaveRequest(
              id: id,
              startDateStr: group.first.dateStr,
              endDateStr: group.last.dateStr,
              ownerId: me.id,
              ownerPhone: me.phone,
              ownerName: me.fullName,
            ),
  ...
  Future<String?> requestLeave(String dateStr) =>
      requestLeaveRange(dateStr, dateStr);

  Future<String?> requestLeaveRange(
    String startDateStr,
    String endDateStr,
  ) async {
    final me = currentUser;
    if (me == null) return 'Session expirée.';
    if (_isDateInPast(startDateStr))
      return 'Une demande de congé ne peut pas commencer dans le passé.';
    if (endDateStr.compareTo(startDateStr) < 0)
      return 'La période de congé est invalide.';
    final overlap = _leaveRequests.any(
      (r) =>
          r.ownerPhone == me.phone &&
          (r.status == LeaveRequestStatus.pendingAdmin ||
              r.status == LeaveRequestStatus.approved) &&
          startDateStr.compareTo(r.endDateStr) <= 0 &&
          endDateStr.compareTo(r.startDateStr) >= 0,
    );
    if (overlap)
      return 'Une demande de congé existe déjà sur tout ou partie de cette période.';
    ...
    if (backendEnabled) {
      try {
        ...
        return 'Demande de congé impossible : $e';
      }
    }
  ...
  Future<String?> cancelLeave(String id) async {
    ...
    if (backendEnabled) {
      try {
        await SupabaseBackendService.instance.cancelLeaveRequest(id);
        _firePush('leave_cancelled', id);
        await _reloadFromBackend();
        return null;
      } catch (e) {
        return 'Annulation impossible : $e';
      }
    }
  ...
  Future<String?> reviewLeave(String id, bool approve) async {
    ...
    if (me == null || me.role != UserRole.admin)
      return 'Action réservée à l’administrateur.';
    final r = _leaveRequests.where((x) => x.id == id).firstOrNull;
    if (r == null || r.status != LeaveRequestStatus.pendingAdmin)
      return 'Demande déjà traitée.';
    if (r.ownerId == me.id || r.ownerPhone == me.phone)
      return 'Un administrateur ne peut pas valider sa propre demande de congé.';
    if (approve) {
      final conflicts = leaveConflictingEntries(r);
      if (conflicts.isNotEmpty) {
        final dates = conflicts
            .map(
              (e) => DateFormat(
                'dd/MM',
                'fr_FR',
    ...
    if (backendEnabled) {
      try {
        await SupabaseBackendService.instance.reviewLeaveRequest(
          id,
          approve ? 'approve' : 'reject',
        );
        _firePush('leave_reviewed', id);
        await _reloadFromBackend();
        return null;
      } catch (e) {
        return 'Traitement impossible : $e';
      }
    }
    final linked = _planning
        .where((e) => e.leaveRequestId == r.id && e.shiftId == 'conge')
        .toList();
    if (approve) {
      if (linked.isEmpty) {
        ...
        while (!d.isAfter(end)) {
          ...
          final existing = _planning
              .where(
                (e) =>
                    (e.ownerId == r.ownerId || e.ownerPhone == r.ownerPhone) &&
                    e.dateStr == dateStr,
              )
              .firstOrNull;
          if (existing == null) {
      ...
      _planning.removeWhere(
        (e) => e.leaveRequestId == r.id && e.shiftId == 'conge',
      );
  ...
  Future<String?> adminReopenPlanningMonth(
    AppUser doctor,
    DateTime month, {
    ...
    final currentFirst = DateTime(now.year, now.month, 1);
    if (first.isBefore(currentFirst))
      return 'Un calendrier d’un mois passé ne peut pas être rouvert.';
    final record = planningMonthForUser(doctor.id, month.year, month.month);
    if (record?.status != PlanningMonthStatus.approved)
      return 'Ce calendrier n’est pas validé.';
    try {
      if (backendEnabled) {
        ...
        final prefix =
            '${month.year}-${month.month.toString().padLeft(2, '0')}-';
        final monthEntries = _planning
            .where(
              (e) => e.ownerId == doctor.id && e.dateStr.startsWith(prefix),
            )
            .toList();
        final entryIds = monthEntries.map((e) => e.id).toSet();
        for (final request in _exchanges.where(
          (x) =>
              _isActiveRequest(x) &&
              (entryIds.contains(x.planningEntryId) ||
                  (x.targetPlanningEntryId != null &&
                      entryIds.contains(x.targetPlanningEntryId))),
        )) {
      ...
      return 'Dévalidation impossible : $e';
    }
  }

  Future<String?> adminDeleteShift(
    String entryId, {
    required String reason,
  }) async {
    final me = currentUser;
    ...
    if (!canAdminDeleteEntry(entry))
      return 'Seules les gardes disciplinaires, les gardes validées et les congés validés peuvent être supprimés par l’administrateur.';
    final cleanReason = reason.trim();
    if (cleanReason.length < 3) return 'Indiquez un motif de suppression.';
    try {
      if (backendEnabled) {
        await SupabaseBackendService.instance.adminDeletePlanning(
          entryId,
          cleanReason,
        );
  ...
  Future<String?> reviewAccount(
    ...
    if (me == null || me.role != UserRole.admin)
      return 'Action réservée à l’administrateur.';
    if (profileId == me.id)
      return 'Vous ne pouvez pas modifier votre propre statut.';
    try {
      if (backendEnabled) {
        await SupabaseBackendService.instance.reviewAccount(
          profileId,
        ...
        if (approve) {
          ...
          if (approvedProfile != null) {
            try {
              final changed = await _syncOfficialRosterForProfile(
                approvedProfile,
              );
      ...
      return 'Validation du compte impossible : $e';
    }
  }
  ...
  DateTime _guardStart(String date, String shiftId) {
    ...
    final p = date.split('-').map(int.parse).toList(),
        s = shift.start!.split(':').map(int.parse).toList();
    return DateTime(p[0], p[1], p[2], s[0], s[1]);
  }

  List<ReminderNotification> _appendReminderInstances(
    String owner,
    String date,
    String shiftId,
  ) {
    final shift = ShiftCatalog.byId(shiftId);
    if (!shift.hasSchedule) return const <ReminderNotification>[];
    final start = _guardStart(date, shiftId);
    final label =
        '${shift.id.startsWith('urg') ? 'Urgences' : 'Service'} · ${shift.label}';
    ...
    for (final delay in _reminderDelays) {
      ...
      for (var repeat = 0; repeat <= repeatLimit; repeat++) {
        ...
        final r = ReminderNotification(
          ...
          label: '$label · $suffix',
          fireAt: fire,
          status: fire.isBefore(DateTime.now())
              ? ReminderStatus.sent
  ...
  String _guardReminderBody(
    ...
    late String when;
    if (days <= 0) {
      when = start.hour >= 17 ? 'ce soir' : 'aujourd’hui';
    } else if (days == 1) {
      when = 'demain';
    } else {
      when = 'dans $days jours';
    }
  ...
  Future<void> _scheduleNativeReminder(ReminderNotification r) async {
    ...
    final e = _planning
        ...
        .firstOrNull;
    if (e == null || !isPlanningEntryApproved(e)) return;
    final sh = ShiftCatalog.byId(e.shiftId);
    if (!sh.hasSchedule) return;
    await NotificationService.instance.scheduleReminder(
      ownerPhone: me.phone,
      dateStr: r.dateStr,
      notificationKey:
          '${e.id}|${e.shiftId}|${r.fireAt.millisecondsSinceEpoch}',
      title: 'Attention · GardeFlow',
      body: _guardReminderBody(r, e, sh),
      fireAt: r.fireAt,
      soundMode: _reminderSoundMode,
      vibration: _reminderVibration,
    );
  ...
  void _scheduleReminder(String owner, String date, String shiftId) {
    ...
    if (currentUser?.phone == owner && _notificationsOn) {
      for (final r in added) {
        unawaited(_scheduleNativeReminder(r));
      }
    }
  }
  ...
  Future<void> rescheduleAllReminders() async {
    ...
    for (final e in _planning.where(
      (e) => e.ownerPhone == me.phone && isPlanningEntryApproved(e),
    )) {
      _appendReminderInstances(me.phone, e.dateStr, e.shiftId);
    }
    await _persistNow();
  ...
  Future<void> _activateNativeRemindersForCurrentUser({
  ...
  void _refreshReminderStatuses() {
    for (final r in _reminders) {
      if (r.fireAt.isBefore(DateTime.now())) r.status = ReminderStatus.sent;
    }
  }
  ...
  bool _isActiveRequest(ExchangeRequest x) =>
      x.status == ExchangeStatus.pendingB ||
  ...
  List<PlanningEntry> exchangeableEntriesFor(
    String phone, {
    String? excludingDate,
    ...
    final result = _planning
        .where(
          (e) =>
              (e.ownerId == phone || e.ownerPhone == phone) &&
              !e.isDisciplinary &&
              ShiftCatalog.byId(e.shiftId).hasSchedule &&
              (!serviceOnly || e.shiftId.startsWith('service-')) &&
              isPlanningEntryApproved(e) &&
              !_isEntryLocked(e.id) &&
              !_guardHasStarted(e),
        )
        .toList();
  ...
  Future<String?> createTransferRequest(
    ...
    final me = currentUser;
    if (me == null) return 'Session expirée.';
    if (entry.ownerId != me.id && entry.ownerPhone != me.phone)
      return 'Cette garde ne vous appartient plus.';
    if (entry.isDisciplinary)
      return 'Une garde disciplinaire ne peut être ni transférée ni échangée.';
    if (!isPlanningEntryApproved(entry))
      return 'Votre calendrier doit être validé avant un transfert ou un échange.';
    if (entry.dateStr != date) return 'La date de la garde est incohérente.';
    if (_guardHasStarted(entry))
      return 'Une garde commencée ou passée ne peut plus être transférée.';
    if (BusinessRules.sameHospitalRequired && target.hospital != me.hospital)
      return 'Les transferts sont limités au même établissement.';
    ...
    if (_isEntryLocked(entry.id))
      return 'Une demande est déjà en cours pour cette garde.';
    if (_planning.any(
      (e) =>
          e.dateStr == date &&
    ...
    final ex = ExchangeRequest(
      id: backendEnabled
          ...
          : 'ex${++_exchangeCounter}',
      type: ShiftRequestType.transfer,
    ...
    if (backendEnabled) {
      try {
        await SupabaseBackendService.instance.createRequest(ex);
        _firePush('exchange_created', ex.id);
      } catch (e) {
  ...
  Future<String?> createSwapRequest(
    ...
    PlanningEntry entry,
    DirectoryContact target,
    PlanningEntry targetEntry,
  ) async {
    final me = currentUser;
    if (me == null) return 'Session expirée.';
    if (entry.ownerId != me.id && entry.ownerPhone != me.phone)
      return 'Cette garde ne vous appartient plus.';
    if (entry.isDisciplinary || targetEntry.isDisciplinary)
      return 'Une garde disciplinaire ne peut être ni transférée ni échangée.';
    if (!isPlanningEntryApproved(entry) ||
        !isPlanningEntryApproved(targetEntry))
      return 'Les deux calendriers doivent être validés avant un échange.';
    if (_guardHasStarted(entry))
      return 'Une garde commencée ou passée ne peut plus être échangée.';
    if (_guardHasStarted(targetEntry))
      return 'La garde choisie est déjà commencée ou passée.';
    if (BusinessRules.sameHospitalRequired && target.hospital != me.hospital)
      return 'Les échanges sont limités au même établissement.';
    ...
    if (targetEntry.ownerId != target.id &&
      ...
      return 'La garde choisie n’appartient plus au médecin destinataire.';
    if (!isUserPlanningMonthApproved(me.id, targetEntry.dateStr) ||
        !isUserPlanningMonthApproved(target.id, entry.dateStr))
      return 'Les mois de destination des deux médecins doivent aussi être validés.';
    if (entry.id == targetEntry.id) return 'Choisissez deux gardes distinctes.';
    if (_isEntryLocked(entry.id) || _isEntryLocked(targetEntry.id))
      return 'Une des deux gardes est déjà engagée dans une demande.';
    final targetConflict = _planning
        .where(
          (e) =>
        ...
        .firstOrNull;
    if (targetConflict != null && targetConflict.id != targetEntry.id)
      return '${target.name} a déjà une autre affectation le ${entry.dateStr}.';
    final sourceConflict = _planning
        .where(
          (e) =>
              e.dateStr == targetEntry.dateStr &&
              (e.ownerId == me.id || e.ownerPhone == me.phone),
        )
        .firstOrNull;
    if (sourceConflict != null && sourceConflict.id != entry.id)
      return 'Vous avez déjà une autre affectation le ${targetEntry.dateStr}.';
    final ex = ExchangeRequest(
      id: backendEnabled
          ? 'ex${DateTime.now().microsecondsSinceEpoch}'
          : 'ex${++_exchangeCounter}',
      ...
      shiftId: entry.shiftId,
      targetPlanningEntryId: targetEntry.id,
      targetDateStr: targetEntry.dateStr,
  ...
  String? _validateRequestState(ExchangeRequest ex) {
    ...
    if (source.isDisciplinary)
      return 'La garde proposée est disciplinaire et ne peut pas être transférée ou échangée.';
    if (!isPlanningEntryApproved(source))
      return 'Le calendrier de la garde source n’est pas validé.';
    if (_guardHasStarted(source))
      return 'La garde proposée est déjà commencée ou passée.';
    if (ex.isTransfer) {
      if (!isUserPlanningMonthApproved(ex.toId, ex.dateStr))
        return 'Le calendrier du destinataire n’est pas validé pour ce mois.';
      final conflict = _planning
          .where(
            (e) =>
                e.dateStr == ex.dateStr &&
                (e.ownerId == ex.toId || e.ownerPhone == ex.toPhone),
          )
          .firstOrNull;
      if (conflict != null)
        return '${ex.toName} a déjà une affectation ce jour-là.';
      return null;
    }
    final targetId = ex.targetPlanningEntryId;
    if (targetId == null)
      return 'La deuxième garde de l’échange est introuvable.';
    final target = _planning.where((e) => e.id == targetId).firstOrNull;
    if (target == null ||
        (target.ownerId != ex.toId && target.ownerPhone != ex.toPhone))
      return 'La garde de ${ex.toName} a été modifiée ou supprimée.';
    if (target.isDisciplinary)
      return 'La garde de ${ex.toName} est disciplinaire et ne peut pas être échangée.';
    if (!isPlanningEntryApproved(target))
      return 'Le calendrier de ${ex.toName} n’est pas validé.';
    if (!isUserPlanningMonthApproved(ex.fromId, target.dateStr) ||
        !isUserPlanningMonthApproved(ex.toId, source.dateStr))
      return 'Les mois de destination ne sont pas tous validés.';
    if (_guardHasStarted(target))
      return 'La garde de ${ex.toName} est déjà commencée ou passée.';
    final fromUser = _users
        .where((u) => u.id == ex.fromId || u.phone == ex.fromPhone)
        .firstOrNull;
    final toUser = _users
        .where((u) => u.id == ex.toId || u.phone == ex.toPhone)
        .firstOrNull;
    if (fromUser == null || toUser == null)
      return 'Un des médecins participant à l’échange est introuvable.';
    ...
    if (involvesService && fromUser.service != toUser.service)
      return 'Toute garde de Service ne peut être échangée qu’entre médecins du même service.';
    final conflictTo = _planning
        .where(
          (e) =>
              e.dateStr == source.dateStr &&
              (e.ownerId == ex.toId || e.ownerPhone == ex.toPhone),
        )
        .firstOrNull;
    if (conflictTo != null && conflictTo.id != target.id)
      return '${ex.toName} a déjà une autre affectation le ${source.dateStr}.';
    final conflictFrom = _planning
        .where(
          (e) =>
              e.dateStr == target.dateStr &&
              (e.ownerId == ex.fromId || e.ownerPhone == ex.fromPhone),
        )
        .firstOrNull;
    if (conflictFrom != null && conflictFrom.id != source.id)
      return '${ex.fromName} a déjà une autre affectation le ${target.dateStr}.';
    return null;
  }
  ...
  Future<String?> acceptExchange(String id) async {
    ...
    if (backendEnabled) {
      try {
        ...
        return 'Acceptation impossible : $e';
      }
    }
  ...
  Future<String?> declineExchange(String id) async {
    ...
    if (backendEnabled) {
      try {
        await SupabaseBackendService.instance.respondRequest(id, 'decline');
        _firePush('exchange_declined', id);
        await _reloadFromBackend();
        return null;
      } catch (e) {
        return 'Refus impossible : $e';
      }
    }
    _updateExchange(id, ExchangeStatus.declinedB);
    return null;
  }
  ...
  Future<String?> rejectExchange(String id) async {
    ...
    if (ex.fromId == me!.id ||
        ...
        ex.toPhone == me.phone)
      return 'Un administrateur participant à la demande ne peut pas la valider.';
    if (backendEnabled) {
      try {
        await SupabaseBackendService.instance.reviewRequest(id, 'reject');
        _firePush('exchange_reviewed', id);
        await _reloadFromBackend();
        return null;
      } catch (e) {
        return 'Rejet impossible : $e';
      }
    }
    _updateExchange(id, ExchangeStatus.rejectedAdmin);
    return null;
  }
  ...
  Future<String?> cancelExchange(String id) async {
    ...
    if (backendEnabled) {
      try {
        ...
        _firePush('exchange_cancelled', id);
        await _reloadFromBackend();
        return null;
      } catch (e) {
        return 'Annulation impossible : $e';
  ...
  void _updateExchange(String id, ExchangeStatus status) {
    final ex = _exchanges.where((e) => e.id == id).firstOrNull;
    if (ex == null) return;
    ex.status = status;
    _persist();
    notifyListeners();
  ...
  int exchangeActionableCount() {
    ...
    return _exchanges
        .where(
          (e) =>
              (e.status == ExchangeStatus.pendingB &&
                  (e.toId == me.id || e.toPhone == me.phone)) ||
              (e.status == ExchangeStatus.pendingAdmin &&
                  me.role == UserRole.admin &&
                  e.fromId != me.id &&
                  e.toId != me.id &&
  ...
  int leaveActionableCount() {
    final me = currentUser;
    if (me == null || me.role != UserRole.admin) return 0;
    return _leaveRequests
        .where(
          (r) =>
              r.status == LeaveRequestStatus.pendingAdmin &&
  ...
  int get accountActionableCount {
    final me = currentUser;
    if (me == null || me.role != UserRole.admin) return 0;
  ...
  bool isNotificationDismissed(String key) =>
      _dismissedNotificationKeys.contains(_scopedNotificationKey(key));
  ...
  void clearReadAndPastNotifications() {
    ...
    for (final e in _exchanges) {
      ...
      if (relevant && terminal)
        _dismissedNotificationKeys.add(
          _scopedNotificationKey('exchange:${e.id}'),
        );
    }
    for (final r in _leaveRequests) {
      ...
      if (relevant && r.status != LeaveRequestStatus.pendingAdmin)
        _dismissedNotificationKeys.add(_scopedNotificationKey('leave:${r.id}'));
    }
    _persist();
    notifyListeners();
  }
  ...
  void _restore(Map<String, dynamic> j) {
    try {
      _users.addAll(
        (j['users'] as List? ?? []).map(
          (e) => AppUser.fromJson(Map<String, dynamic>.from(e)),
        ),
      );
    } catch (_) {}
    try {
      _planning.addAll(
        (j['planning'] as List? ?? []).map(
          (e) => PlanningEntry.fromJson(Map<String, dynamic>.from(e)),
        ),
      );
    } catch (_) {}
    try {
      _astreinte.addAll(
        (j['astreinte'] as List? ?? []).map(
          (e) => AstreintePhoto.fromJson(Map<String, dynamic>.from(e)),
        ),
      );
    } catch (_) {}
    try {
      _reminders.addAll(
        (j['reminders'] as List? ?? []).map(
          (e) => ReminderNotification.fromJson(Map<String, dynamic>.from(e)),
        ),
      );
    } catch (_) {}
    try {
      _manualDirectoryContacts.addAll(
        (j['manualDirectoryContacts'] as List? ?? []).map(
          (e) => DirectoryContact.fromJson(Map<String, dynamic>.from(e)),
        ),
      );
    } catch (_) {}
    try {
      _exchanges.addAll(
        (j['exchanges'] as List? ?? []).map(
          (e) => ExchangeRequest.fromJson(Map<String, dynamic>.from(e)),
        ),
      );
    } catch (_) {}
    try {
      _leaveRequests.addAll(
        (j['leaveRequests'] as List? ?? []).map(
          (e) => LeaveRequest.fromJson(Map<String, dynamic>.from(e)),
        ),
      );
    } catch (_) {}
    try {
      _planningMonths.addAll(
        (j['planningMonths'] as List? ?? []).map(
          (e) => PlanningMonth.fromJson(Map<String, dynamic>.from(e)),
        ),
      );
    } catch (_) {}
        ...
        if (seen.add(key))
          _planningMonths.add(
            PlanningMonth(
              ...
              year: d.year,
              month: d.month,
              status: PlanningMonthStatus.approved,
            ),
          );
      }
    }
    ...
    try {
      ...
      for (final item in raw.entries) {
        _reminderPrefsByUser[item.key] = Map<String, dynamic>.from(
          item.value as Map,
        );
      }
    } catch (_) {}
    _planningCounter =
        (j['planningCounter'] as num?)?.toInt() ?? _planning.length;
    _exchangeCounter =
        (j['exchangeCounter'] as num?)?.toInt() ?? _exchanges.length;
    _leaveCounter =
        (j['leaveCounter'] as num?)?.toInt() ?? _leaveRequests.length;
    _reminderCounter =
        (j['reminderCounter'] as num?)?.toInt() ?? _reminders.length;
  }
